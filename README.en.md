...
The working version belongs to the `4.4.2` series: major version 4 is retained while the latest functional changes and fixes are consolidated before the next minor/patch number is fixed.
...
The documented version is `4.4.2` on the `main` branch. This is a personal-use project; each platform and its APIs are subject to their own terms of service.
